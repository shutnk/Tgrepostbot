...
echo "🚀 Начинаю полное исправление..."

# 1. Удаляем старые сессии
rm -f session.session session.b64

# 2. Создаём новую сессию через пароль (без QR)
cat > create_session.py << 'PYEOF'
...
# 3. Запускаем создание сессии
python create_session.py

# 4. Кодируем сессию
cat session.session | base64 -w 0 > session.b64

# 5. Создаём новый bot.py (без ошибок, с новой сессией)
cat > bot.py << 'PYEOF'
import os
import base64
import asyncio
...
# 6. Пушим всё на GitHub
git add session.b64 bot.py requirements.txt
git commit -m "Final fix: fresh session via password, no QR"
git push -u origin main --force

echo "✅ ВСЁ ГОТОВО! Зайди на Render и нажми Manual Deploy."
