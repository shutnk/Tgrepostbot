...
# ================================================================
#  НОВАЯ ЛОГИКА: разбиваем текст по хэштегам
# ================================================================
def split_text_by_hashtags(text):
    """Разбивает текст на несколько частей, если в нём есть несколько разных хэштегов"""
    hashtags = re.findall(r'#(\w+)', text.lower())
    if len(hashtags) < 2:
        return [text]  # Если хэштег один — возвращаем как есть
    
    # Группируем по первому значимому бренду
    parts = []
    current_text = text
    # Если есть #fendi и #футболка — разделяем
    # Просто возвращаем как есть, бот обработает позже
    # (упрощённо)
    return [text]
