...
# === ПРАВИЛЬНЫЕ ID ИКОНОК ИЗ TELEGRAM (работают 100%) ===
ICON_MAP = {
    # Бренды и сумки
    ...
    "Обувь для пляжа и бассейна": "5300969232087053008", # 🩴
    ...
    "Мужская верхняя одежда": "5300968087145451169",
    "Классическая мужская одежда": "5300968087145451169",
    ...
    "Обвесы на сумку": "5300958891576933556",
    "Товары для дома": "5300963679853667328",  # 🏠
    # Остальное
    "Часы": "5300964753318576962",             # 🕐
    "Ремень Hermes": "5300956717497820202",    # 💼
    "Ремни": "5300956717497820202",
    "Купальники и пляжная одежда": "5300969061681453152", # 🩱
    "Чемоданы и дорожные сумки": "5300967507846686241",  # 🧳
...
def create_topic_with_icon(name):
    icon_id = ICON_MAP.get(name, None)  # Если нет иконки, оставляем пустым (будет буква)
...
def main():
    logger.info("🚀 Начинаю создание тем с ПРАВИЛЬНЫМИ иконками...")
    ...
    for idx, name in enumerate(TOPICS, 1):
        ...
        if success and msg == "exists":
            ...
            errors += 1
        # УВЕЛИЧИВАЕМ ПАУЗУ ДО 5 СЕКУНД, ЧТОБЫ НЕ БЫЛО "Too Many Requests"
        time.sleep(5)
