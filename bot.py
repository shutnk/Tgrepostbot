        ...
        if 't.me/' in text and '|' in text:
            try:
                ...
                if is_posted(msg_id, source_chat.id):
                    await update.message.reply_text(f"⏭️ Пост {msg_id} уже был скопирован.")
                    return
                
                # Заменяем текст
                new_text = re.sub(r'@\w+', NEW_AUTHOR, msg.caption or msg.text or "")
                
                # Отправляем в целевой канал (в нужную тему)
                # Если бот админ в канале, он сможет отправить в тему по названию
                sent_msg = await msg.copy(
                    chat_id=TARGET_CHANNEL,
                    caption=new_text,
                    message_thread_id=topic_name  # Название темы
                )
                
                # Сохраняем в базу
                save_posted(msg_id, sent_msg.message_id, source_chat.id)
                
                await update.message.reply_text(f"✅ Пост {msg_id} скопирован в тему '{topic_name}'!")
                print(f"✅ Переслан пост {msg_id} в тему {topic_name}")
                
            except Exception as e:
                await update.message.reply_text(f"❌ Ошибка: {e}")
                print(f"❌ Ошибка: {e}")
        elif 't.me/' in text:
            # Если ссылка без темы, просто пересылаем как есть
            try:
                link = text.strip()
                msg_id = int(link.split('/')[-1])
                msg = await context.bot.get_message(chat_id=SOURCE_CHANNEL, message_id=msg_id)
                ...
                source_chat = await context.bot.get_chat(SOURCE_CHANNEL)
                if is_posted(msg_id, source_chat.id):
                    await update.message.reply_text(f"⏭️ Уже скопировано.")
                    return
                
                new_text = re.sub(r'@\w+', NEW_AUTHOR, msg.caption or msg.text or "")
                sent_msg = await msg.copy(chat_id=TARGET_CHANNEL, caption=new_text)
                save_posted(msg_id, sent_msg.message_id, source_chat.id)
                await update.message.reply_text(f"✅ Пост скопирован!")
...
print("🚀 Бот запущен! Жду ссылки с темами от скрипта.")
